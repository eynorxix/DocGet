...
Genera un documento .docx. Debes tener el skill_id y pedir title, author, tutor, content_input al usuario.
...
SYSTEM_PROMPT = f"""Eres un asistente conversacional integrado en DocGent, un generador de documentos .docx académicos.
...
4. Si la herramienta requiere parámetros que el usuario no dio, pídelos primero.
5. Para generar documentos, si el usuario no ha seleccionado una skill, usa la skill "indice" por defecto. Solo usa `request_skill_select()` si el usuario explícitamente pide elegir otra skill.
6. Si el usuario quiere generar pero no dio title/author/tutor, pídelos.
7. IMPORTANTE: No inventes IDs de skills. Siempre lista primero o pide el nombre.
8. Cuando crees contenido para una skill o documento, sé detallado y profesional. Usa `web_search()` para obtener información actualizada de internet cuando sea relevante para mejorar la calidad del contenido.

SKILL POR DEFECTO:
Existe una skill llamada "Índice General" (ID: indice) con una estructura genérica de índice académico (introducción, objetivos, marco teórico, aplicación, conclusiones, bibliografía, anexos). Úsala como predeterminada cuando el usuario quiera generar un documento pero no haya seleccionado una skill específica. Especialmente útil si el usuario da solo un tema sin especificar estructura.
...
11. Cuando el usuario tenga una skill seleccionada y sea apropiada, pregúntale: "¿Está todo listo para generar el documento? Responde Y (sí) o N (no)."
12. Si el usuario responde "Y" o "y", ejecuta generate_document con los parámetros. Si faltan datos como title/author/tutor, pídelos antes. Si el usuario responde "N" o "n", espera.

DATOS DE CARÁTULA (enviados automáticamente por el sistema):
13. El usuario puede tener datos de carátula guardados (título, autor, tutor). Si ves "SISTEMA: Datos de carátula disponibles:", esos datos ya están completos. Úsalos directamente en generate_document sin preguntar.
14. Si no ves datos de carátula o faltan campos (están vacíos), pídelos al usuario como siempre.
"""
...
def _execute_tool(name: str, params: dict, api_keys: list[str] | None = None) -> tuple[str, dict | None]:
    try:
        if name == "list_skills":
            ...
            content = gemini_service.generate_document_content(
                ...
                user_input=params.get("content_input", ""),
                api_keys=api_keys,
            )
            logo = None
            logo_path = Path(os.path.expanduser("~")) / "Documentos" / "Biblioteca" / "logoUnifranz" / "logounifranz.png"
            if logo_path.exists():
                logo = str(logo_path)
...
def process_message(message: str, history: list[dict], api_keys: list[str] | None = None, caratula: dict | None = None) -> dict:
    ...
    if match:
        try:
            ...
            result_msg, data = _execute_tool(tool_name, tool_params, api_keys=api_keys)
